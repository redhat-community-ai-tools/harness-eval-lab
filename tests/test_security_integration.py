"""Integration tests for eval-setup-security CLI command."""
...
def test_security_command_terminal(tmp_path: Path) -> None:
    ...
    runner = CliRunner()
    result = runner.invoke(cli, ["eval-setup-security", str(setup)])
...
def test_security_command_json(tmp_path: Path) -> None:
    setup = _create_setup(tmp_path)
    runner = CliRunner()
    result = runner.invoke(cli, ["eval-setup-security", str(setup), "--format", "json"])
    assert result.exit_code == 0
    import json
...
def test_security_command_detects_issues(tmp_path: Path) -> None:
    ...
    output = json.loads(result.output)
    total_errors = output["inspection"]["summary"]["errors"]
...
def test_security_command_fail_on_error(tmp_path: Path) -> None:
    setup = _create_setup(tmp_path)
    runner = CliRunner()
    result = runner.invoke(cli, ["eval-setup-security", str(setup), "--fail-on-error"])

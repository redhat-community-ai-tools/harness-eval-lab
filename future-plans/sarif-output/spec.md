...
harness-eval-lab produces findings in two formats: terminal (human-readable) and JSON (machine-readable). Neither integrates natively with GitHub's code scanning experience. Users who run harness-eval-lab in CI see findings in a log, not inline on the PR diff. This means findings are easy to miss and hard to act on.
...
| harness-eval-lab concept | SARIF field |
|---|---|
...
Add a `format_sarif()` function in `src/harness_eval_lab/output/sarif.py` that converts `list[Finding]` to a SARIF v2.1.0 document. The SARIF spec is well-defined; the mapping is straightforward since findings already carry rule ID, severity, message, file path, and line number.
...
**Given** a team has harness-eval-lab in their CI pipeline
